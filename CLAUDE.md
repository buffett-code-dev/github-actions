...
└── github/                 # GitHub関連のアクション
    ...
    ├── label-stale-items/      # (非推奨) 古いissue/PRへのラベル付与のみ
...
- **label-stale-items** (非推奨、互換維持のためのみ残置):
  - staleラベル付与のみ、自動closeは行わない (`days-before-*-close` のデフォルトは `-1`)
  - 新規利用は `manage-stale-items` を推奨
- **manage-stale-items**:
